...
interface SupplierSummary { label: string; value: number }
...
function SupplierDetailContent({ year, setYear, period, setPeriod }: { year: number; setYear: (y: number) => void; period: Period; setPeriod: (p: Period) => void }) {
  ...
  const summary = data?.["supplierSummary"] as SupplierSummary | null
  const spendByYear = (data?.["supplierSpendByYear"] as BarDataPoint[] | null) ?? []
  const recentInvoices = (data?.["supplierRecentInvoices"] as SupplierInvoice[] | null) ?? []
  const jobs = (data?.["supplierJobs"] as SupplierJob[] | null) ?? []
        ...
        <MotionList className="widget-grid widget-grid-2">
          ...
          <MotionItem className="col-span-full">
            <Widget title="Spend History" loading={isLoading} noData={!isLoading && spendByYear.length === 0}>
              <Chart config={{ type: "line", series: [{ id: "Spend", color: "#16a34a", data: spendByYear.map((d) => ({ x: d.label, y: d.value })) }], enableArea: true }} />
            </Widget>
          </MotionItem>
...
export default function SupplierDetailPage() {
  ...
  if (numericId === null || isNaN(numericId)) {
    return <Page title="Supplier Not Found" />
  }

  return (
    <PageDataProvider
      module="suppliers"
      queries={PAGE_QUERIES.supplierDetail}
      params={{ year, id: numericId, ...periodToParams(period) }}
    >
      <SupplierDetailContent year={year} setYear={setYear} period={period} setPeriod={setPeriod} />
    </PageDataProvider>
  )
}
