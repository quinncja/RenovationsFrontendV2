...
function SuppliersContent({ year, setYear, period, setPeriod }: { year: number | null; setYear: (y: number | null) => void; period: Period; setPeriod: (p: Period) => void }) {
  const isMobile = useIsMobile()
  const navigate = useNavigate()
  const { data, isLoading } = useWidgetData(["allMaterialSuppliersBySpend"])
  const items = (data?.["allMaterialSuppliersBySpend"] as SpendItem[] | null) ?? []
  ...
  return (
    <Page title="Vendors" actions={<><PeriodSelector value={period} onChange={setPeriod} disabled={year === null} /><YearSelector allowAllTime value={year} onChange={setYear} /></>}>
                ...
                <tbody>
                  {sorted.map((item) => (
                    <tr
                      ...
                      role="button"
                      tabIndex={0}
                      onKeyDown={e => e.key === "Enter" && navigate(`/suppliers/${item.id}`)}
                    >
...
export default function SuppliersPage() {
  ...
  return (
    <PageDataProvider module="suppliers" queries={PAGE_QUERIES.suppliers} params={{ year, ...(year !== null ? periodToParams(period) : {}) }}>
      <SuppliersContent year={year} setYear={setYear} period={period} setPeriod={setPeriod} />
    </PageDataProvider>
  )
}
