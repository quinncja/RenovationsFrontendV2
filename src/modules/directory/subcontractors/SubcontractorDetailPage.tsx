...
export default function SubcontractorDetailPage() {
  ...
  return (
    <PageDataProvider module="dashboard" queries={PAGE_QUERIES.subcontractorDetail} params={{ id: numericId }}>
      <SubcontractorDetail />
    </PageDataProvider>
  )
}
...
function SubcontractorDetail() {
  const { data, isLoading } = useWidgetData<{
    subcontractorSummary: { name: string; totalSpend: number; jobCount: number } | null
    subcontractorSpendByYear: { year: number; spend: number }[] | null
    subcontractorRecentInvoices: { id: number; date: string; amount: number; description: string }[] | null
    subcontractorJobs: { recnum: number; jobnme: string; spend: number }[] | null
  }>(["subcontractorSummary", "subcontractorSpendByYear", "subcontractorRecentInvoices", "subcontractorJobs"])

  const summary = data?.subcontractorSummary
  const invoices = data?.subcontractorRecentInvoices
  const jobs = data?.subcontractorJobs

  return (
    <Page title={summary?.name ?? "Subcontractor"}>
