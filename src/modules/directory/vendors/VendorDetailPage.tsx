...
// Shared accent colors / icons with ClientDetailPage / SubcontractorDetailPage
// so the three directory detail pages all read the same. Mirrors the global
...
export default function VendorDetailPage() {
  ...
  // VendorsPage shares this key and persists `null` for "All Time". Detail
  // `null` = All Time (the backend omits the year filter when null).
  ...
  return (
    <PageDataProvider module="dashboard" queries={PAGE_QUERIES.vendorDetail} params={{ id: numericId, year }}>
      <VendorDetail year={year} onYearChange={setYear} />
    </PageDataProvider>
  )
}

function VendorDetail({ year, onYearChange }: { year: number | null; onYearChange: (y: number | null) => void }) {
  ...
  const { data, isLoading } = useWidgetData<{
    vendorSummary: Summary | null
    vendorSpendByYear: YearPoint[] | null
    vendorRecentInvoices: RecentInvoice[] | null
    vendorJobs: VendorJob[] | null
  }>(["vendorSummary", "vendorSpendByYear", "vendorRecentInvoices", "vendorJobs"])

  const summary = data?.vendorSummary ?? null
  const spendByYear = data?.vendorSpendByYear ?? []
  ...
  return (
    <Page
      title={summary?.label ?? "Vendor"}
      actions={<YearSelector value={year} onChange={onYearChange} allowAllTime />}
    >
      <MotionList className="widget-grid widget-grid-2">
        <MotionItem>
          <StatWidget title={`${year ?? "All-Time"} Material Spend`} value={summary?.value ?? null} loading={isLoading} />
        </MotionItem>
        <MotionItem>
          <StatWidget
            title="All-Time Material Spend"
