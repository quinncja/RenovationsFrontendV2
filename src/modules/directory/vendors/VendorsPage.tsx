...
function VendorsContent({ year, onYearChange }: { year: number | null; onYearChange: (y: number | null) => void }) {
  ...
  // Share of the whole list's spend — the concentration read ("our top three
  // vendors are half of material spend").
        ...
        <MotionItem>
          <Widget loading={isLoading} noData={!isLoading && items.length === 0} className="co-widget">
            <div className="co-widget-toolbar">
              <SearchField variant="co" placeholder="Search vendors..." value={search} onChange={setSearch} />
              ...
              <table className="spend-rank-table">
                <thead>
                  <tr>
                    ...
                    <SortTh spendRank col="label" label="Vendor Name" sortKey={sortKey} sortDir={sortDir} onSort={handleSort} />
                ...
                <tbody>
                  {sorted.map((item) => (
                    <tr
                      ...
                      onClick={() => goToPartner("vendor", item.id)}
                      role="button"
                      tabIndex={0}
                      onKeyDown={(e) => e.key === "Enter" && goToPartner("vendor", item.id)}
                    >
