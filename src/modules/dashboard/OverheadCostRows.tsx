          ...
          <div key={row.key} data-row-key={row.key} className={`ohr-cost-card${isOpen ? " ohr-cost-card-open" : ""}`}>
            ...
            <AnimatePresence initial={false}>
              {isOpen && (
                ...
                >
                  <CostItems items={row.items ?? "loading"} showMonth={showMonth} showCategory={showCategory} onOpen={onOpen} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          {sorted.map((li, i) => {
            ...
            return (
              <motion.tr
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1, transition: { delay: Math.min(i, 8) * 0.012 + 0.04, duration: 0.14 } }}
              >
                ...
                {showCategory && <td>{String(collapseValue(li.category) ?? "") || "—"}</td>}
