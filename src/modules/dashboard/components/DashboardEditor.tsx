...
export function DashboardEditor() {
  const {
    editLayout, moveWidget, moveWidgetToEnd, moveWidgetToStart,
  ...
  const widgets = editLayout.widgets
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (overId === START_ZONE_ID) {
      isMulti ? moveWidgetsToStart(movingIds) : moveWidgetToStart(activeId)
    } else if (overId === END_ZONE_ID) {
      isMulti ? moveWidgetsToEnd(movingIds) : moveWidgetToEnd(activeId)
      ...
      if (!isMulti && activeId === anchorId) {
        ...
        // Insert after the anchor widget
        isMulti ? insertWidgets(movingIds, anchorId, "after") : insertWidget(activeId, anchorId, "after")
      }
      ...
      if (!isMulti && activeId === targetWidgetId) {
        ...
        // Insert before the offset widget (insertWidget clears the target's offset)
        isMulti ? insertWidgets(movingIds, targetWidgetId, "before") : insertWidget(activeId, targetWidgetId, "before")
      }
    } else {
      const targetId = overId as WidgetId
      isMulti ? moveWidgets(movingIds, targetId) : moveWidget(activeId, targetId)
    }
