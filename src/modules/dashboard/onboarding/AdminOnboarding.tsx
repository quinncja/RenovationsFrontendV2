...
// App-level host for the rich admin first-run tour (phases 0–2 full-screen
// takeover → phase 3, a single coachmark over the live dashboard highlighting
...
// again. Picking a layout (phase 2) calls chooseTemplate → completeSetup, which
...
export function useAdminOnboardingTour(): { navbarVeil: NavbarVeil; tour: ReactNode } {
  ...
  // Intro advance (phases 0→1, 1→2).
  const onAdvance = () => setStage((s) => (s === 0 ? 1 : 2))
  const onSkip = () => setStage(2)
  ...
  const onPick = (template: LayoutTemplate) => {
    ...
    setStage(3)
    // Phase 3 anchors on /dashboard (the gear lives there via
    // DashboardHeaderActions). Deep-link admins got phases 0–2 anywhere; move
  ...
  const navbarVeil: NavbarVeil =
    typeof stage === "number" && stage <= 2 ? "veiled" : stage === 3 ? "unveiling" : "off"
