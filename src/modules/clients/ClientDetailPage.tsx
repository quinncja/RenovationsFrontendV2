...
interface ClientSummary { label: string; value: number }
interface ClientInvoice {
  id: string; jobName: string; invoiceNum: string; description: string | null
  value: number; invoiceDate: unknown; status: number; amountRemaining: number
}
interface ClientJob {
  recnum: number; jobName: string; status: number; revenue: number
...
function ClientDetailContent({ year, setYear }: { year: number; setYear: (y: number) => void }) {
  ...
  const summary = data?.["clientSummary"] as ClientSummary | null
  const revenueByYear = (data?.["clientRevenueByYear"] as BarDataPoint[] | null) ?? []
  const recentInvoices = (data?.["clientRecentInvoices"] as ClientInvoice[] | null) ?? []
  const jobs = (data?.["clientJobs"] as ClientJob[] | null) ?? []
        ...
        <MotionList className="widget-grid widget-grid-2">
          ...
          <MotionItem className="col-span-full">
            <Widget title="Revenue History" loading={isLoading} noData={!isLoading && revenueByYear.length === 0}>
              <Chart config={{ type: "line", series: [{ id: "Revenue", color: "#1f78c5", data: revenueByYear.map((d) => ({ x: d.label, y: d.value })) }], enableArea: true }} />
            </Widget>
          </MotionItem>
...
export default function ClientDetailPage() {
  ...
  if (numericId === null || isNaN(numericId)) {
    return <Page title="Client Not Found" />
  }
